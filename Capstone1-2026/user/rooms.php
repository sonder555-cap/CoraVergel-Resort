...
function openModal(roomId, price, img){
    /* No dates — go to dashboard booking section to pick dates first */
    if (!HAS_DATES) {
        window.location.href = '../user/dashboard.php?room=' + encodeURIComponent(roomId) + '#booking-section';
        return;
    }
    modalPrice = price;
...
window.addEventListener('load',()=>{ document.getElementById('pageAlert')?.scrollIntoView({behavior:'smooth',block:'center'}); });
<?php endif; ?>
/* Auto-open modal if coming back from dashboard with room + dates */
