...
function getRoomInfo($conn, $room_name) {
    $stmt = $conn->prepare("SELECT room_id, room_name, price, total_units, capacity, description, image FROM rooms WHERE room_name = ?");
...
function countOverlappingBookingsByStatus($conn, $room_name, $check_in, $check_out, $status, $exclude_booking_id = null) {
    // Count reserved room units, not just booking rows. Existing rows without
    // room_count are treated as one unit for backward compatibility.
    $sql = "SELECT COALESCE(SUM(COALESCE(room_count,1)),0) c FROM bookings
            WHERE room_type = ?
              AND status = ?
              AND check_in < ?
              AND check_out > ?";
 ...
 */
function isRoomAvailable($conn, $room_name, $check_in, $check_out, $exclude_booking_id = null, $units = 1) {
    $available = getAvailableUnits($conn, $room_name, $check_in, $check_out, $exclude_booking_id);
    if ($available === null) return false;
    return $available >= max(1, (int)$units);
}