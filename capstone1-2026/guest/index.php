...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'contact') {
    ...
    $subject_in = trim($_POST['subject'] ?? '');
    $message_in = trim($_POST['message'] ?? '');

    if (empty($first_name) || empty($last_name) || empty($message_in)) {
        ...
        $safe_subject = $subject_in !== '' ? htmlspecialchars($subject_in, ENT_QUOTES, 'UTF-8') : 'New inquiry from website';
        $safe_message = nl2br(htmlspecialchars($message_in, ENT_QUOTES, 'UTF-8'));
        $safe_phone   = htmlspecialchars($phone, ENT_QUOTES, 'UTF-8');

        $bodyHtml = "<p><strong>From:</strong> {$full_name}</p>"
                  . "<p><strong>Email:</strong> {$email}</p>"
                  . ($safe_phone !== '' ? "<p><strong>Phone:</strong> {$safe_phone}</p>" : "")
                  . "<p><strong>Message:</strong></p><p>{$safe_message}</p>";

        // Goes to the resort's own inbox — same address shown in the topbar/footer.
        $sent = sendMail('coravergelresort@gmail.com', 'CoraVergel Resort', "Website Inquiry: {$safe_subject}", $bodyHtml);

        if ($sent) {
            $contact_success = "Thanks, {$full_name}! Your message has been sent — we'll get back to you soon.";
        } else {
            ...
            <div class="bbar-wrap" id="bbarWrap">
                ...
                <div class="bbar-benefits">
                    <div class="bbar-benefit">
                        ...
                        <div class="bbar-benefit-txt">Get more savings when you book direct! <a href="deals.php" class="bbar-benefit-link">Learn More</a></div>
                    </div>
                </div>
            </div>
        </div>
...
<section class="contact-section" id="contact">
    <div class="contact-split">
        ...
        <div class="contact-visual">
            <div class="contact-visual-overlay"></div>
            <div class="contact-visual-content">
                <span class="cv-eyebrow">Get In Touch</span>
                <h2 class="cv-title">We'd Love to<br>Hear <em>From You</em></h2>
                <p class="cv-sub">Whether you're planning a getaway or have a question about your stay, our team in Tigbauan is ready to help.</p>

                <div class="cv-divider"></div>

                <div class="cv-info-list">
                    <a href="tel:3202512" class="cv-info-row">
                        <span class="cv-info-icon"><i class="fa-solid fa-phone"></i></span>
                        <span class="cv-info-text"><span class="cv-info-lbl">Call Us</span><span class="cv-info-val">+320 2512</span></span>
                    </a>
                    <a href="mailto:coravergelresort@gmail.com" class="cv-info-row">
                        <span class="cv-info-icon"><i class="fa-solid fa-envelope"></i></span>
                        <span class="cv-info-text"><span class="cv-info-lbl">Email Us</span><span class="cv-info-val">coravergelresort@gmail.com</span></span>
                    </a>
                    <a href="https://www.google.com/maps/@10.714106,122.396162,16z" target="_blank" rel="noopener noreferrer" class="cv-info-row">
                        <span class="cv-info-icon"><i class="fa-solid fa-location-dot"></i></span>
                        <span class="cv-info-text"><span class="cv-info-lbl">Visit Us</span><span class="cv-info-val">5021 Barosong, Tigbauan, Iloilo</span></span>
                    </a>
                </div>
        ...
        <div class="contact-form-panel">
            <span class="cfp-eyebrow">Send a Message</span>
            <h3 class="cfp-title">How can we help?</h3>

            <?php if ($contact_success): ?>
            <div class="cfp-alert success"><i class="fa-solid fa-circle-check"></i> <?= $contact_success ?></div>
            <?php endif; ?>
            <?php if ($contact_error): ?>
            <div class="cfp-alert error"><i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($contact_error) ?></div>
            <?php endif; ?>
    ...
    <div class="footer-top">
        <div class="footer-brand">
            <div class="footer-logo-wrap">
                <img src="../assets/images/logo/cv_logo.png" width="100" height="100" alt="CoraVergel Resort Logo" class="footer-logo-img">
            </div>
        </div>
