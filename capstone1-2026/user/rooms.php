...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'book_room') {
    ...
    if (empty($room_type) || empty($check_in) || empty($check_out) || $guests < 1) {
        ...
        if (empty($booking_error)) {
            ...
            $stmt = $conn->prepare("INSERT INTO bookings (booking_ref, room_type, check_in, check_out, guests, adults, children, total_price, guest_name, guest_email, id_type, id_photo, contact_number, payment_method, payment_reference, payment_receipt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param(
    "ssssiiidssssssss",
    $booking_ref,
    $room_type,
            ...
            <div class="modal-scroll">
                ...
                <div class="cf-field">
                    ...
                    <select id="paymentMethod" name="payment_method" required onchange="handlePaymentMethodChange()">
                        ...
                        <option value="E-wallet">GCash</option>
                ...
                <div class="cf-field" id="paymentReceiptField" style="display:none;">
                    <label for="paymentReceiptInput">Upload screenshot of your receipt</label>
                    <label class="id-upload-wrap" id="receiptUploadWrap" for="paymentReceiptInput">
                        <i class="fa-solid fa-receipt"></i>
                        <div class="id-upload-label" id="receiptUploadLabel">Tap to upload your payment receipt</div>
                    </label>
                    <input type="file" id="paymentReceiptInput" name="payment_receipt" accept=".jpg,.jpeg,.png,.webp,image/*" onchange="handleReceiptChange()">
                </div>
            </div>
        ...
        <div class="modal-right">
            <div class="modal-scroll">
                <div class="modal-no-dates">
                    ...
                    <a href="index.php#booking-section" class="modal-submit" style="margin-top:8px;">
...
══════════════════════════════════════ */
const PAYMENT_INFO = {
    'E-wallet': 'Send your deposit via GCash to <strong>+320 2512</strong>, then enter your reference number below. We\'ll verify it before confirming your booking.',
