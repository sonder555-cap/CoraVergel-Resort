...
if (isset($_GET['confirm_booking'])) {
    ...
    $stmt = $conn->prepare("SELECT room_type, check_in, check_out, guests, total_price, guest_name, guest_email, status FROM bookings WHERE booking_id = ?");
    $stmt->bind_param("i", $bid);
    $stmt->execute();
...
/* ── Bookings ──
   ...
   on the booking form. ── */
$bookings = [];
$bq = $conn->query("SELECT booking_id, guest_name full_name, guest_email, id_type, id_photo, contact_number, room_type, check_in, check_out, guests, status, created_at, COALESCE(total_price,0) total_price
    FROM bookings
...
<div class="table-controls">
    ...
    <button class="date-clear-btn" id="dateFilterClear" onclick="clearDateFilter(event)" style="display:none;">
        <i class="fa-solid fa-rotate-left"></i> Clear dates
    </button>
</div>
            </div>
...
<?php
    ...
    $bm_view_call = "openBookingModal('$bm_name','$bm_room','$bm_checkin','$bm_checkout',$nights,{$b['guests']},'$bm_booked','$bm_email','$bm_idtype','$bm_contact','{$b['status']}','$bm_idphoto')";
?>
                            <td>
        ...
        adminPicker=flatpickr('#adminDateRange',{
            ...
            onChange:function(d){
                if(d.length===2){
                    ...
                    document.getElementById('dateFilterClear').style.display='inline-flex';
...
function clearDateFilter(e){
    e.stopPropagation();
    dfpFrom=''; dfpTo='';
    if(adminPicker) adminPicker.clear();
...
function openBookingModal(name,room,checkin,checkout,nights,guests,booked,email,idType,contact,status,idPhoto){
...
document.addEventListener('keydown',e=>{ if(e.key==='Escape'){ document.getElementById('searchDropdown').classList.remove('open'); document.getElementById('globalSearch').value=''; closeBookingModal(); closeIdPhotoLightbox(); if(adminPicker) adminPicker.close(); } });
